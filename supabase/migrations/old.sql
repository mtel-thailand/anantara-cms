...
begin
  ...
  with base as (
    ...
    where
      case
        ...
        when p_status = 'archived'::public.submission_status then true
        else v.archived_at is null
      end
      and case
        when p_status is null then v.status not in ('approved'::public.submission_status, 'finalized'::public.submission_status)
  ...
  filtered_counts as (
    ...
    where
      (
        (v.status = 'archived'::public.submission_status)
  ...
  select jsonb_object_agg(
    ...
    (
      select count(*)
      from public.car_submission_vehicles all_v
      inner join public.car_submissions_form all_f on all_f.id = all_v.submission_id
      where
        all_v.status not in ('approved'::public.submission_status, 'finalized'::public.submission_status)
        and case
          when v_keyword is null then true
          when v_keyword ~* '^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$' then
            all_v.id::text = v_keyword or all_v.car_id::text = v_keyword
          else not exists (
            select 1
            from unnest(v_keywords) as keyword_part(term)
            where not (
              all_v.make_of_vehicle ilike '%' || replace(replace(keyword_part.term, '%', '\%'), '_', '\_') || '%' escape '\'
              or all_v.car_id::text ilike '%' || replace(replace(keyword_part.term, '%', '\%'), '_', '\_') || '%' escape '\'
              or all_f.first_name ilike '%' || replace(replace(keyword_part.term, '%', '\%'), '_', '\_') || '%' escape '\'
              or all_f.name ilike '%' || replace(replace(keyword_part.term, '%', '\%'), '_', '\_') || '%' escape '\'
              or btrim(concat_ws(' ', all_f.first_name, all_f.name)) ilike '%' || replace(replace(keyword_part.term, '%', '\%'), '_', '\_') || '%' escape '\'
              )
          )
        end
    )
