    ...
    jsonb_build_object(
      ...
      'cars', coalesce(
        (
          ...
          where car.category_id is not null
            or (
...
declare
  ...
  managed_car_ids text[];
  assignable_car_ids text[];
  payload_car_ids text[] := '{}'::text[];
...
begin
  ...
  select coalesce(array_agg(car.id::text order by car.id::text), '{}'::text[])
    into managed_car_ids
  from public.cars as car
  join public.car_submission_vehicles as vehicle
    on vehicle.id = car.submission_vehicle_id
  where car.category_id is not null
  ...
  loop
    ...
    payload_car_ids := array_append(payload_car_ids, car_payload ->> 'id');
  end loop;

  if cardinality(payload_car_ids) <> cardinality(managed_car_ids)
    or exists (
      select 1
      from unnest(payload_car_ids) as payload_id
      where not payload_id = any(managed_car_ids)
    )
    or cardinality(payload_car_ids) <> (
      select count(distinct payload_id)
      from unnest(payload_car_ids) as payload_id
    ) then
    raise exception 'The assignable car list changed. Refresh and try again.';
  end if;
  ...
  loop
    ...
    if car_payload ->> 'id' = any(assignable_car_ids) then
      continue;
    end if;

    select car.category_id
      into v_current_category_id
    from public.cars as car
    where car.id::text = car_payload ->> 'id';

    if v_category_id is not null
      and (
        v_category_id !~ '^[0-9]+$'
        or v_current_category_id is distinct from v_category_id::bigint
      ) then
      raise exception 'Only finalized cars can be assigned to a different class.';
    end if;
  end loop;
  ...
  if exists (
    ...
    from jsonb_array_elements(p_classes)
    where value ->> 'id' ~ '^temp-'
      and coalesce((value ->> 'removed')::boolean, false) is false
  ) then
    perform setval(
      pg_get_serial_sequence('public.car_categories', 'id')::regclass,
      coalesce((select max(id)::bigint from public.car_categories), 1),
      exists (select 1 from public.car_categories)
    );
  end if;
